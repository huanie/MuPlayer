...
pub fn scan_directory<P: AsRef<Path>>(root: P, pool: SqlitePool) -> (usize, Vec<error::ScanError>) {
    ...
    let (oks, errs): (Vec<()>, Vec<error::ScanError>) = WalkDir::new(root)
        ...
        .partition_map(|x| {
            ...
            macro_rules! return_tag {
                (NO_NUM, $x:expr) => {
                    $x.chars().map_while(|c| c.to_digit(10)).fold(0, |acc, digit| acc * 10 + digit)
                };
                (UNKNOWN, $x:expr) => {
                    $x
                };
            ...
            let artist = album_artist.map_or(track_artist, |x| {
                x.text().unwrap_or(track_artist)
            });
            ...
            if track_number == 1 && disc_number == 1 {
                ...
                transaction
                    .prepare_cached(
                        "INSERT INTO album (album_title, artist_name) VALUES (?1, ?2)",
                    )
                    .unwrap()
...
INSERT OR IGNORE
INTO song (disc_number, path, title, modified_stamp, artist_name, track_number, duration, album_title)
VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)
"#).unwrap().execute(rusqlite::params![disc_number, path.to_string_lossy(), title, mtime, artist, track_number, duration, album]).unwrap();
...
fn extract_metadata<'a>(
    tagged: &'a TaggedFile,
    path: &Path,
) -> Result<Metadata<'a>, error::ScanError> {
