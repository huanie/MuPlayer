...
class SearchModel: ObservableObject {
  ...
  init() {
    self.text = ""
    self.debouncedText = ""
    $text.debounce(for: .seconds(0.75), scheduler: RunLoop.main).assign(to: &$debouncedText)
  }
}
        ...
        Table(songs, selection: $selected) {
          TableColumn("") {
            Image(systemName: "play.fill")
              .opacity($0.rowid == self.mpv.currentSong?.rowid ? 1 : 0)
          }.width(max: 10)
          TableColumn("Nr") {
          ...
          if let rowid = self.selected {
            let song = try! database.read {
              try! Song.fetchOne(
                $0,
                sql: "SELECT \(Song.columns()) FROM song WHERE rowid = ?",
                arguments: [rowid])!
            }
